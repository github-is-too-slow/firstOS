 ...
 * 加入truncate操作模式，即保留文件头inode，但是可以覆盖掉文件的数据信息
...
PUBLIC int do_open()
{
	...
	for (i = 0; i < NR_FILE_DESC; i++)
		if (f_desc_table[i].fd_inode == 0)  //找到了,fd_inode表示inode的指针
			break;
	if (i >= NR_FILE_DESC)
	...
	if (inode_nr == INVALID_INODE) { /* file not exists */
		...
		if ((flags & O_CREATE) && (!(flags & O_TRUNC))) {
			assert(flags == (O_RDWR | O_CREATE));
			printf("{FS} file exists: %s\n", pathname);
			return -1;
		}
		assert((flags ==  O_RDWR                     ) ||
		       (flags == (O_RDWR | O_TRUNC          )) ||
		       (flags == (O_RDWR | O_TRUNC | O_CREATE)));
	...
	if (pin) {//此时要么是新创建文件的iNode，要么是旧文件的iNode
		...
		// printf("strip failure %d", inode_nr);
		return -1;
	}
