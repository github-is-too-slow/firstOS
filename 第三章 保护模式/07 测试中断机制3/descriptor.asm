...
[bits 32]
LABEL_IDT:
%rep 32 ; 目标段选择子，       段内偏移，          Dcount，        属性
        Gate    SelectorCode32,     SpuriousHandler,    0,          DA_386IGate
%endrep
.020h:  Gate    SelectorCode32,     ClockHandler,        0,          DA_386IGate ;20h号时钟中断描述符
%rep 95
        Gate    SelectorCode32,     SpuriousHandler,    0,          DA_386IGate
%endrep
.080h:  Gate    SelectorCode32,     UserHandler,        0,          DA_386IGate ;80h号中断描述符
...
LABEL_SEG_CODE32:
    ...
    call Init8259A      ;初始化8259A中断控制器，此时相当于开启了保护模式下的中断机制
...
ClockHandler equ _ClockHandler - $$
    inc byte [gs:((80 * 0 + 75) * 2)]  ;每一次时钟中断使显存单元变为下一个ASCII字符
    mov al, 20h
    out 20h, al     ;向主8259A中断控制器发送EOF，20h对应的EOF位为1
    iretd
