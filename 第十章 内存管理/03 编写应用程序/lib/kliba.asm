...
;功能：INS / INSB / INSW / INSD-从端口到字符串的输入
;原型：void port_read(u16 port, void *buf, int n);
;指令介绍：任何两个连续的8位端口都可以视为16位端口，而任何四个连续的端口都可以视为32位端口。
;INSW从连续IO端口空间读取16位值到指定的内存地址。INSW将从DX中指定的PORT读取一个字节，
;从PORT + 1读取另一个字节，这些字节分别存储在ES：E（DI）和ES：E（DI）+1。
;REP将重复此过程E（CX）中指定的次数。
port_read:
    mov edx, [esp + 4]  ;port
    mov edi, [esp + 8]  ;buf
    mov ecx, [esp + 12]  ;n，以字节为单位
    shr ecx, 1
    cld     ;清空DF位
    rep insw
    ret

;功能：rep outsw 从内存字符串到端口的输出
;原型：void port_write(u16 port, void *buf, int n);
port_write:
    mov edx, [esp + 4]  ;port
    mov esi, [esp + 8]  ;buf
    mov ecx, [esp + 12]  ;n，以字节为单位
