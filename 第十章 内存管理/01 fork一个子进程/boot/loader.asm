...
LABEL_LOADERING_FILE:
    ...
    add bx, [BPB_BytesPerSec]   ;每次读取一个扇区，内核存储位置偏移512字节
    jc .overflow        ;bx进位变成了0，存储位置超出了16位，即内核文件大小超出了64k
    jmp .notoverflow   ;没有超出，正常继续读取
.overflow:
    push ax
    mov ax, es
    add ax, 1000h  ;es原本指向0x70000处，加上1000h,便指向下一个段0x80000处，此时bx=0
    mov es, ax
    pop ax
.notoverflow:
...
LABEL_PM_START:
	...
	mov	eax, [dwMemSize]
	mov	[BOOT_PARAM_ADDR + 4], eax ; memory size内存最大地址
	mov	eax, BaseOfKernelFile  ;内核基址
	shl	eax, 4
	add	eax, OffsetOfKernelFile    ;内核偏移
	mov	[BOOT_PARAM_ADDR + 8], eax ; phy-addr of kernel.bin绝对物理地址
