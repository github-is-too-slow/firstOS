...
%macro hwint_slave 1
    ...
    ;此处可以加上关闭相同类型中断的代码
    in al, INT_S_CTLMASK
    or al, (1 << (%1 - 8))        ;置1关闭对应的中断
    out INT_S_CTLMASK, al
    ;中断结束标志,提前结束中断
    mov al, EOI
    out INT_M_CTL, al           ;向主i8259A芯片发送中断结束标志
    nop         ;连续两个io操作延迟一会
    out INT_S_CTL, al           ;向从i8259A芯片发送中断结束标志
    ...
    ;此处可以加上根据中断处理逻辑的返回值是否开启相同类型中断的代码
    in al, INT_S_CTLMASK
    and al, ~(1 << (%1 - 8))        ;开启对应中断，一元操作符~作用是：取反
    out INT_S_CTLMASK, al
    ret   ;弹出内核栈顶中的eip,要么是restart，要么是restart_reenter
%endmacro
...
sys_call:
    ...
    sti
    ;此时切换到了内核堆栈
    push esi        ;esi此时指向进程表顶部
    push dword [p_proc_ready]  ;将用户进程表的地址入栈，第4个参数入栈
    push edx    ;第3个参数入栈
    push ecx    ;第2个参数入栈
    push ebx    ;第1个参数入栈
    ;处理逻辑
